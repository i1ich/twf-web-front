...
export const getFields = (
  ...
  const computationalGoalType: ComputationGoalType = watch(
    `tasks[${index}].computationGoalType`
  );
  const reductionGoalType: ReductionGoalType = watch(
    `tasks[${index}].reductionGoalType`
  );

  const inputs = [
    ...getSubjectTaskTypeFields(subjectType),
    ...getMainConditionsFields(
      subjectType,
      taskType,
      computationalGoalType,
      reductionGoalType
    ),
  ...
  return inputs.map((input) => {
    return { ...input, name: `tasks[${index}].${input.name}` };
  });
};
