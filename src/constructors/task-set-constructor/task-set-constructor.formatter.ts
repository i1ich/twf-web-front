    ...
    return {
      ...
      tasks: data.tasks.map((task: TaskConstructorReceivedForm) => {
        ...
        const otherGoalDataDefault =
          CONSTRUCTOR_JSONS_INITIAL_STATE.taskSet.tasks[0].otherGoalData;

        taskCopy.otherGoalData = {
          ...otherGoalDataDefault,
          ...taskCopy.otherGoalData,
        };

        taskCopy.otherGoalData.hiddenGoalExpressions =
          taskCopy.otherGoalData.hiddenGoalExpressions.map((expr: any) => {
            if (expr.format) {
              return expr;
            }
            return {
        ...
        if (taskCopy.rules) {
          taskCopy.rules = taskCopy.rules.map(
          ...
          if (taskCopy.rules.length === 0) {
            taskCopy.rules =
              CONSTRUCTOR_JSONS_INITIAL_STATE.taskSet.tasks[0].rules;
          }
        }
    ...
    if (data.tasks) {
      res.tasks = data.tasks.map((task: TaskConstructorInputs) => {
        ...
        if (task.taskType === TaskType.REDUCTION) {
          fields = computationAdditionalFields[
            task.computationGoalType as ComputationGoalType
          ].map((field) => {
            return field.name.replace("otherGoalData.", "");
          });
        } else if (task.taskType === TaskType.COMPUTATION) {
          fields = reductionAdditionalFields[
            task.reductionGoalType as ReductionGoalType
          ].map((field) => {
            return field.name.replace("otherGoalData.", "");
          });
        }
        ...
        if (taskCopy.originalExpression.format === MathInputFormat.TEX) {
          taskCopy.originalExpressionTex =
          ...
          taskCopy.originalExpression.format === MathInputFormat.PLAIN_TEXT
        ) {
          taskCopy.originalExpressionPlainText =
            taskCopy.originalExpression.expression;
          ...
          taskCopy.originalExpression.format ===
          MathInputFormat.STRUCTURE_STRING
        ) {
          taskCopy.originalExpressionStructureString =
            taskCopy.originalExpression.expression;
          taskCopy.originalExpressionPlainText = convertMathInput(
            MathInputFormat.STRUCTURE_STRING,
            MathInputFormat.PLAIN_TEXT,
        ...
        if (taskCopy.goalExpression != null) {
          if (!taskCopy.goalExpression.expression) {
            ...
            if (taskCopy.goalExpression.format === MathInputFormat.TEX) {
              ...
              taskCopy.goalExpression.format === MathInputFormat.PLAIN_TEXT
            ) {
              taskCopy.goalExpressionPlainText =
                taskCopy.goalExpression.expression;
              ...
              taskCopy.goalExpression.format ===
              MathInputFormat.STRUCTURE_STRING
            ) {
              taskCopy.goalExpressionStructureString =
                taskCopy.goalExpression.expression;
              taskCopy.goalExpressionPlainText = convertMathInput(
                MathInputFormat.STRUCTURE_STRING,
                MathInputFormat.PLAIN_TEXT,
        ...
        [
          ...
          if ((task as any)[key] === "") {
            ...
            console.log(
              `field ${key} is not null and blank, it is: `,
              (task as any)[key]
            );
        ...
        if (task.rules) {
          taskCopy.rules = task.rules
            .filter((rule: RuleConstructorInputs) => {
              return (
                rule.left &&
                rule.right &&
                rule.left.expression.trim() &&
                rule.right.expression.trim()
              );
            })
